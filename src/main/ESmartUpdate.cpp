...
void ESmartUpdate::handleUpdate(String jsonStr) {
    ...
    config.update = Updates{version};

    strcpy(config.update.url, url.c_str());
    strcpy(config.update.host, host.c_str());
    DEBUG_PRINT(config.update.host);
    DEBUG_PRINTLN(config.update.url);
    saveUpdates(config.update);
    checkForUpdate();
}
...
void ESmartUpdate::checkForUpdate() {
    DEBUG_PRINT(config.update.host);
    DEBUG_PRINTLN(config.update.url);
    if (VERSION < config.update.version) {
        DEBUG_PRINT("Update found old version: ");
        DEBUG_PRINT(VERSION);
        DEBUG_PRINT(" new version: ");
        DEBUG_PRINTLN(config.update.url);
        startUpdate(config.update.host, config.update.url);
    }
}
