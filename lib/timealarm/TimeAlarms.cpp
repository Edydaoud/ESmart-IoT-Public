...
void TimeAlarmsClass::write(AlarmID_t ID, time_t value) {
    if (isAllocated(ID) && Alarm[ID].value != value) {
...
uint8_t TimeAlarmsClass::getDigitsNow(dtUnits_t Units) const {
    time_t time = now();
    if (Units == dtSecond)
        return numberOfSeconds(time);
    if (Units == dtMinute)
        return numberOfMinutes(time);
    if (Units == dtHour)
        return numberOfHours(time);
    if (Units == dtDay)
...
AlarmID_t TimeAlarmsClass::create(time_t value, OnTick_t onTickHandler, uint8_t isOneShot, dtAlarmPeriod_t alarmType, int index, FutureJob work) {
    if (!((dtIsAlarm(alarmType) && now() < SECS_PER_YEAR) || (dtUseAbsoluteValue(alarmType) && (value == 0)))) {
        if (index != -1) {
            if (Alarm[index].Mode.alarmType == dtNotAllocated) {
                // here if there is an Alarm id that is not allocated
                Alarm[index].onTickHandler = onTickHandler;
